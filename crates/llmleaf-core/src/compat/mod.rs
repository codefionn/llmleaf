...
//! [`anthropic`] is a second chat dialect on the same canonical core — the Anthropic Messages API,
//! mapped at the edge exactly like the rest (principle 3: no dialect is privileged). Routing treats
//! them uniformly — a logical model resolves to the same fallback chain whatever the dialect or
//! modality (batch excepted: it targets one provider and does not fall back — see [`batch`]).
//!
