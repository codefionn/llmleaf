...
//! drives the core only through narrow, HTTP-free handles: the [`KeyStore`] snapshot-swap methods and
//! the [`EventBus`] tap. Three jobs, each independently enabled by config:
//!   - **PULL** identity ([`control.identity`]) and verdicts ([`control.limits`]) on an interval and
//!     install them as node-local snapshots. Fail-open (keep last-good) for limits; fail-closed on
//!     cold start for identity (an empty identity cache must reject, never admit).
...
use llmleaf_core::{ControlConfig, EventBus, InterceptPhase, Interceptor, KeyStore, ResolvedAuth};
...
pub use refresh::{IdentityRefresher, VerdictRefresher};
